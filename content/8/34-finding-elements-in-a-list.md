...
# Пошук елементів у списку: методи index() та count()
...
Перед тим як шукати номер слота через `index()`, варто переконатися, що річ взагасти є в наявності.
...
- Якщо є — виведіть його номер слота та кількість (використовубючи функції `index()` та `count()`).
...
- Якщо кількість ресурсу "Wood" більша за 5 то:
...
3. Впевнись, що можеш пояснити свіій код.
