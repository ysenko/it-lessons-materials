...
Функція `max()` Миттєво знаходить найбільше число. А функція `min()` - найменьше.
...
Легенда: Ви пишете скрипт для аналізу здобичі після рейду. `gold_drops` (скопіюйте в код стрічку нижче), що описує всі купи золота, які випали з монстрів:
...
В інвентарі мало місця. Напишіть алгоритм (цикл `for`), який знайде суму лише тих куп золота, які є меншими за `100` (дріб'язок).
