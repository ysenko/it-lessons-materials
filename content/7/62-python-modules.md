...
1. Запитайте у користувача радіус кола за допомогою `input`.
2. Перетворіть введене значення у дробове число (`float`).
3. Обчисліть площу кола: $S = math.pi \cdot r^2$.
4. Знайдіть квадратний корінь із введеного радіуса за допомогою `math.sqrt()`.
